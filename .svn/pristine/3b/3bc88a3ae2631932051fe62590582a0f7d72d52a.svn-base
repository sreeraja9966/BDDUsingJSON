...
public class WebTableHelper extends TestBase {
    ...
    RepositoryHelper repositoryHelper = AutomationObjectFactory.getRepositoryHelperInstance();
        ...
        try {
            ...
            for (int i = 1; i <= row.size(); i++) {
                ...
                for (int j = 1; j <= col.size(); j++) {
                    headerValue = findElement
                            .searchClickableElement(By.xpath((xpath + "/table/thead/tr/th[" + j + "]"))).getText()
                            .replaceAll("\\s+", "");
                    colValue = findElement
                            .searchClickableElement(By.xpath(xpath + "/table/tbody/tr[" + i + "]/td[" + j + "]"))
     ...
     * 
     * 
     * try { List<WebElement> col = d.findElements(By.xpath(xpath+"/table/thead/tr/th")); List<WebElement> row =
     * d.findElements(By.xpath(xpath+"/table/tbody/tr/td[1]")); File directory = new File(relativePath()+locatorFile);
     * // int fileCount=directory.list().length; List<String> propertyFiles = new ArrayList<>(); for(File
     * file:directory.listFiles()) { if(file.getName().endsWith(".properties")) { propertyFiles.add(file.getName()); } }
     * AutoPicking autoPicking = new AutoPicking(); map=autoPicking.loadingPropertyFile(locatorFile,
     * (propertyFiles.size()-2)); leftColValues=new ArrayList<>(); rightColValues=new ArrayList<>(); for (String name:
     * map.keySet()){
     * 
     * key =name.toString().split("_");
     * 
     * key1=key[0]; key2=key[1]; leftColValues.add(key1); rightColValues.add(key2); } String headerValue=null; String
     * headerValueWithoutSpace=null;
     * 
     * log.info("no.of col--------> "+col.size()); log.info("no.of row--------> "+row.size());
     * 
     * tableDatafromPropertyFile=new ArrayList<>();
     * 
     * for(int j=1;j<=col.size();j++) { headerValue=findElement.searchClickableElement(By.xpath((xpath+
     * "/table/thead/tr/th["+j+"]"))).getText(); log.info(j+headerValue+"-------->headerValue"); headerValueWithoutSpace
     * =headerValue.replaceAll("\\s+",""); log.info(headerValueWithoutSpace+
     * "==========================>headerValueWithoutSpace"); for(int k=1;k<leftColValues.size();k++) {
     * 
     * if (!"label".equalsIgnoreCase(rightColValues.get(k))) {
     * if((leftColValues.get(k).equalsIgnoreCase(headerValueWithoutSpace))) {
     * log.info(k+pro.getProperty(headerValueWithoutSpace+"_"+(rightColValues.get(k)
     * ))+"===========> Value from property file"); tableDatafromPropertyFile.add(headerValue+"="+pro.getProperty(
     * headerValueWithoutSpace+"_"+(rightColValues.get(k)))); } } } } } catch (Exception e) { // TODO Auto-generated
    ...
    /*
     * @param it require a filePath where the expected result is listed 1) this should be a property file 2) it should
     * contain all expected result row wise 3) the should be saved in another folder(not in locator.properties file
     * folder)
     */
    ...
    public void validateTableValues(String xpath, String locatorFile, Properties pro, int i) {
        ...
        if (!"".equals(locatorFile)) {
            try {
                ...
                /*
                 * assertionHelper.verifyListOfElements(getDataFromWebTable(xpath), getTableDataFromPropertyFile(xpath,
                 * locatorFile,pro));
                 */
            } catch (Error e) {
                log.error("values not matched");
            }
        }
    }
