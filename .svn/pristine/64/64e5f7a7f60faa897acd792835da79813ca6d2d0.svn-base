...
public class JsWaiter extends TestBase {
    ...
    public static void setDriver(WebDriver driver) {
        ...
        jsWait = new WebDriverWait(d, 10);
        jsExec = (JavascriptExecutor) d;
    }
    ...
    private void waitForJQueryLoad() {
        try {
            ExpectedCondition<Boolean> jQueryLoad = WebDriver -> ((Long) ((JavascriptExecutor) d)
    ...
    private void waitUntilJSReady() {
        try {
            ExpectedCondition<Boolean> jsLoad = WebDriver -> ((JavascriptExecutor) d)
        ...
        try {
            ...
            if (angular5Check != null) {
                ...
                if (!angularPageLoaded) {
                    poll(5);
                    angularPageLoaded = (Boolean) jsExec.executeScript(
    ...
    private void angularLoads(String angularReadyScript) {
        try {
            ExpectedCondition<Boolean> angularLoad = WebDriver -> Boolean
                    .valueOf(((JavascriptExecutor) d).executeScript(angularReadyScript).toString());
    ...
    public void waitForElementAreComplete(By by, int expected) {
        ExpectedCondition<Boolean> angularLoad = WebDriver -> {
            int loadingElements = d.findElements(by).size();
    ...
    public void waitForAnimationToComplete(String css) {
        ExpectedCondition<Boolean> angularLoad = WebDriver -> {
            int loadingElements = d.findElements(By.cssSelector(css)).size();
